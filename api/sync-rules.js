  ...
  try {
    switch (req.method) {
      ...
      case "POST": {
        ...
        const ins = await pool.query("INSERT INTO sync_rules (user_id, event, active, message_template, delay_minutes) VALUES ($1,$2,$3,$4,$5)", [targetId, event, active, message_template || null, delay_minutes]);
        const r = await pool.query("SELECT * FROM sync_rules WHERE id = $1", [ins.insertId]);
        await pool.query(`DELETE FROM sync_rules WHERE user_id=$1 AND id NOT IN (SELECT id FROM (SELECT id FROM sync_rules WHERE user_id=$1 ORDER BY created_at DESC LIMIT 100) AS keep)`, [targetId]).catch(() => {});
      ...
      case "PUT": {
        ...
        const upd = await pool.query(`UPDATE sync_rules SET ${fields.join(", ")} WHERE id = $${idx}${ownerFilter}`, values);
        if (!upd.rowCount) return res.status(404).json({ success: false, message: "Regra não encontrada" });
        const r = await pool.query("SELECT * FROM sync_rules WHERE id = $1", [id]);
      ...
      case "PATCH": {
        ...
        const ownerFilter = caller.role === "admin" ? "" : ` AND user_id = ${caller.id}`;
        const upd = await pool.query(`UPDATE sync_rules SET active=$1, updated_at=NOW() WHERE id=$2${ownerFilter}`, [active, id]);
        if (!upd.rowCount) return res.status(404).json({ success: false, message: "Regra não encontrada" });
        const r = await pool.query("SELECT * FROM sync_rules WHERE id = $1", [id]);
        return res.status(200).json({ success: true, message: "Regra atualizada", rule: r.rows[0] });
      }
      case "DELETE": {
        ...
        const { id } = req.query; if (!id) return res.status(400).json({ success: false, message: "id obrigatório" });
        const ownerFilter = caller.role === "admin" ? "" : ` AND user_id = ${caller.id}`;
        const r = await pool.query(`DELETE FROM sync_rules WHERE id=$1${ownerFilter}`, [id]);
