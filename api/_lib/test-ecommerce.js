...
async function testNuvemshop({ store_id, access_token }) {
  ...
  if (res.status === 401 || res.status === 403)
    throw new Error(`Token inválido ou sem permissão (HTTP ${res.status}). Verifique o Access Token.`);
  if (res.status === 404)
    throw new Error(`Loja não encontrada (HTTP 404). Verifique o Store ID "${store_id}".`);
  if (!res.ok)
    throw new Error(`Nuvemshop retornou HTTP ${res.status}: ${JSON.stringify(body).slice(0, 200)}`);
...
async function testShopify({ store_url, api_token, api_version }) {
  if (!store_url || !api_token)
    throw new Error("store_url e api_token são obrigatórios.");
  ...
  if (res.status === 401 || res.status === 403)
    throw new Error(`Token inválido ou sem permissão (HTTP ${res.status}).`);
  if (res.status === 404)
    throw new Error(`Loja não encontrada. Verifique a URL "${store_url}".`);
  if (!res.ok)
    throw new Error(`Shopify retornou HTTP ${res.status}: ${JSON.stringify(body).slice(0, 200)}`);
...
async function testWoocommerce({ site_url, consumer_key, consumer_secret }) {
  ...
  if (res.status === 401 || res.status === 403)
    throw new Error(`Credenciais inválidas (HTTP ${res.status}). Verifique Consumer Key e Secret.`);
  if (res.status === 404)
    throw new Error(`URL não encontrada (HTTP 404). Verifique se o WooCommerce está instalado em "${site_url}".`);
  if (!res.ok)
    throw new Error(`WooCommerce retornou HTTP ${res.status}: ${JSON.stringify(body).slice(0, 200)}`);
...
async function testVtex({ account_name, app_key, app_token }) {
  ...
  if (res.status === 401 || res.status === 403)
    throw new Error(`Credenciais inválidas (HTTP ${res.status}). Verifique App Key e App Token.`);
  if (res.status === 404)
    throw new Error(`Conta não encontrada. Verifique o Account Name "${account_name}".`);
  if (!res.ok)
    throw new Error(`VTEX retornou HTTP ${res.status}: ${JSON.stringify(body).slice(0, 200)}`);

  return { store: account_name, plan: "VTEX", country: null };
}

async function testTray({ api_address, access_token }) {
  if (!api_address || !access_token)
    throw new Error("api_address e access_token são obrigatórios.");
  ...
  if (res.status === 401 || res.status === 403)
    throw new Error(`Token inválido ou sem permissão (HTTP ${res.status}).`);
  if (!res.ok)
    throw new Error(`Tray retornou HTTP ${res.status}: ${JSON.stringify(body).slice(0, 200)}`);
...
async function handler(req, res) {
  ...
  // Helper para notificar admins sobre falha no teste
  ...
  try {
    ...
    const msg = err.name === "TimeoutError"
      ? `Timeout: "${platformLabel}" não respondeu em 10 segundos.`
      : err.message;
