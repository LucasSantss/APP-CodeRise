...
export async function handleSetup(req, res) {
  ...
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS notifications (id INT AUTO_INCREMENT PRIMARY KEY, type VARCHAR(30) NOT NULL, title VARCHAR(100) NOT NULL, message TEXT NOT NULL, image_url TEXT, target_role VARCHAR(20) DEFAULT 'all', target_user_id INT, scheduled_at DATETIME, created_by INT, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (target_user_id) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL);`);
    await pool.query(`CREATE TABLE IF NOT EXISTS notification_reads (notification_id INT NOT NULL, user_id INT NOT NULL, hidden BOOLEAN NOT NULL DEFAULT false, read_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (notification_id, user_id), FOREIGN KEY (notification_id) REFERENCES notifications(id) ON DELETE CASCADE, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE);`);
    await pool.query(`CREATE TABLE IF NOT EXISTS admin_webhook_settings (id SMALLINT PRIMARY KEY DEFAULT 1, webhook_url TEXT, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT admin_webhook_settings_single_row CHECK (id = 1));`);
    ...
    await pool.query(`ALTER TABLE user_integrations ADD COLUMN IF NOT EXISTS chatbot_token VARCHAR(64) UNIQUE`).catch(()=>{});
    await pool.query(`ALTER TABLE user_integrations ADD COLUMN IF NOT EXISTS sync_schedule JSON NOT NULL DEFAULT ('{"enabled": false, "times": [], "timezone": "America/Sao_Paulo"}')`).catch(()=>{});
    await pool.query(`ALTER TABLE user_webhooks ADD COLUMN IF NOT EXISTS source VARCHAR(20) DEFAULT 'ecommerce'`).catch(()=>{});

    // ── Índices de performance (MySQL não tem CREATE INDEX IF NOT EXISTS —
    // o catch(()=>{}) cobre "índice já existe" em execuções repetidas) ───────
    for (const idx of [
    ...
    // ── Seeds (idempotentes via INSERT IGNORE — equivalente ao antigo
