...
// ─── Helpers Suri ─────────────────────────────────────────────────────────────
/** @deprecated Use suriRequestWithRetry do módulo _retry.js */
...
export async function processWebhookJob(userId, jobPayload) {
  const { webhookId, eventType, normalized, suri_endpoint, suri_token, ecommerce_platform, user_id, platformLabel, userName } = jobPayload;
...
// ─── handleWebhook: recebe o request, responde imediatamente, enfileira ──────
export async function handleWebhook(req, res) {
  ...
  let integration;
  try {
    let r = await pool.query("SELECT user_id, suri_active, suri_endpoint, suri_token, ecommerce_platform, ecommerce_config, chatbot_platform, chatbot_config, chatbot_active, chatbot_token, webhook_token FROM user_integrations WHERE webhook_token = $1", [token]);
    if (!r.rows[0]) r = await pool.query("SELECT user_id, suri_active, suri_endpoint, suri_token, ecommerce_platform, ecommerce_config, chatbot_platform, chatbot_config, chatbot_active, chatbot_token, webhook_token FROM user_integrations WHERE chatbot_token = $1", [token]);
  ...
  // Nuvemshop: busca dados completos + variantes
  ...
  try {
    ...
    const ins = await pool.query(
      ...
      [user_id, logEventType, JSON.stringify(rawPayload), webhookSource]
    );
    webhookId = ins.rows[0].id;
    // Mantém apenas os últimos 100 por usuário
    pool.query(`DELETE FROM user_webhooks WHERE user_id=$1 AND id NOT IN (SELECT id FROM user_webhooks WHERE user_id=$1 ORDER BY received_at DESC LIMIT 100)`, [user_id]).catch(() => {});
  } catch (err) { return res.status(500).json({ success: false, message: "Erro ao salvar: " + err.message }); }

  // ── Responde imediatamente (204 = aceito, sem body) ───────────────────────
  // Isso evita timeouts e reentregas da plataforma de e-commerce
  if (!suri_active || !suri_endpoint || !suri_token) {
    return res.status(200).json({ success:true, message:"Evento registrado. Suri não configurada ou inativa.", event_type:eventType, webhook_id:webhookId });
  }

  // ── Enfileira o processamento assíncrono ──────────────────────────────────
  const priority = eventType.startsWith("order") ? 10 : 5; // Pedidos têm prioridade
  ...
  // Notifica o SSE imediatamente (evento "received")
