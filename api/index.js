...
// ════════════════════════════════════════════════════════════════════════════
// FLUXO REVERSO: Chatbot → E-commerce
// ════════════════════════════════════════════════════════════════════════════
...
async function handleWebhook(req, res) {
  ...
  let webhookId;
  try {
    const ins = await pool.query("INSERT INTO user_webhooks (user_id, event_type, payload, status) VALUES ($1, $2, $3, 'received') RETURNING id", [user_id, eventType, JSON.stringify(rawPayload)]);
  ...
  // ── FLUXO REVERSO: Chatbot → E-commerce ──────────────────────────────────
  const { ecommerce_config } = integration;
